...
class NdbSearchTabViewModel: ObservableObject {
    @Published var name: String = ""
    @Published var icao: String = ""
    @Published var region: String = ""
    @Published var searchResults: [NDB] = []
    ...
    func searchVor() {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 0.4, repeats: false, block: { (_) in
            if self.name.count < 4, self.icao.count < 2, self.region.count < 2 {
                ...
                DBReader.shared.asyncSearchNdbForAnnotation(name: self.name, icao: self.icao, region: self.region) { mainList in
