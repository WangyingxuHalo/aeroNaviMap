//
//  NavSearchViewModel.swift
//  AeroNaviMap
//
//  Created by 王迎旭 on 11/2/22.
//

import Foundation

class NavSearchTabViewModel: ObservableObject {
    @Published var icao: String = ""
    @Published var region: String = ""
    @Published var searchResults: [NAV] = []
    @Published var isSearchInvalid = true
    var timer: Timer?

    func searchAirport() {
        timer?.invalidate()
        timer = Timer.scheduledTimer(withTimeInterval: 0.4, repeats: false, block: { (_) in
            if self.icao.count < 2, self.region.count < 2 {
                self.isSearchInvalid = true
                self.searchResults = []
            } else {
                DBReader.shared.asyncSearchNavForAnnotation(icao: self.icao, region: self.region) { mainList in
                    DispatchQueue.main.async {
                        self.isSearchInvalid = false
                        self.searchResults = mainList
                    }
                }
            }
        })
    }
}
